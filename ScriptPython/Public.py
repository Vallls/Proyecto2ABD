...
def Sensor():
    sensor_entrada = pandas.read_sql_query("SELECT  Min(sensor_entrada.idsensor), Max(sensor_entrada.idsensor) From sensor_entrada",conn)
...
def sensor_tiendas(tienda):
    sensor_entrada = pandas.read_sql_query("SELECT  Min(sensor_tienda.idsensor), Max(sensor_tienda.idsensor) From sensor_tienda where sensor_tienda.idtienda = %s"%tienda,conn)
    ini= int(pandas.unique(sensor_entrada['min']))
    fin = int(pandas.unique(sensor_entrada['max']))
    return [ini,fin]
...
def ultimoAcceso(mcadress):
    ultimoAcceso = pandas.read_sql_query("select fecha_hora from sensor_mcadress where mcadress = '%s' order by fecha_hora desc limit 1"%mcadress,conn)
    fecha = str(pandas.unique(ultimoAcceso['fecha_hora']))
    return fecha

def CedulaMA(mcadress):
    cedulaMA = pandas.read_sql_query("SELECT cedula from mcadress where mcadress =  '%s'"%mcadress,conn)
...
def main():
    ...
    while(day != 30):
        ...
        while(personasPorDia>1):
            ...
            print('---------------------Persona Nueva-------------------------')
            horaBase= datetime.datetime.now().replace(hour=0,minute=0, second=0, day=dia, month= mes)
